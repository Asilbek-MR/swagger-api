...
class OrderView(generics.GenericAPIView):
    ...
    def post(self,request):
        ...
        if serializer.is_valid():
            serializer.save(customer=request.data)
            print(f"\n {serializer.data}")
...
class OrderIdView(generics.GenericAPIView):
    ...
    def get(sefl,request,order_id):
        order=get_object_or_404(Order,pk=order_id)
        serializer=sefl.serializer_class(instance=order)
        return Response(data=serializer.data,status=status.HTTP_200_OK)
...
class UpdateOrderStatusView(generics.GenericAPIView):
    ...
    def put(self,request,order_id):
        ...
        serializer=self.serializer_class(instance=order,data=request.user)
...
class UserOrdersView(generics.GenericAPIView):
    ...
    @swagger_auto_schema(operation_summary="Get all order made by a specific user")
